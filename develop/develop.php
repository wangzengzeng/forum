...
function devmessage($message, $url = '', $type = '', $values = array(), $extra = '', $extrajs = '', $confirmedname = 'confirmed') {
	...
	if($type == 'form') {
		$message = "<form method=\"post\" action=\"$url\"><input type=\"hidden\" name=\"formhash\" value=\"".FORMHASH."\">".
			"<br />$message$extra".
			"<p class=\"margintop\"><input type=\"submit\" class=\"btn\" name=\"$confirmedname\" value=\"È·¶¨\"".($extrajs ? ' '.$extrajs : '')."> &nbsp; \n".
			"<script type=\"text/javascript\">".
			"if(history.length > (BROWSER.ie ? 0 : 1)) document.write('<input type=\"button\" class=\"btn\" value=\"È¡Ïû\" onClick=\"history.go(-1);\">');".
			"</script>".
			"</p></form><br />";
		...
		$message = "<form method=\"post\" action=\"$url\" id=\"loadingform\"><input type=\"hidden\" name=\"formhash\" value=\"".FORMHASH."\"><br />$message$extra<img src=\"static/image/admincp/ajax_loader.gif\" class=\"marginbot\" /><br />".
			'<p class="marginbot"><a href="###" onclick="$(\'loadingform\').submit();" class="lightlink">Èç¹ûÄúµÄä¯ÀÀÆ÷Ã»ÓÐ×Ô¶¯Ìø×ª£¬Çëµã»÷ÕâÀï</a></p></form><br /><script type="text/JavaScript">setTimeout("$(\'loadingform\').submit();", 2000);</script>';
	} else {
		...
		if($url) {
			if($type == 'button') {
				...
				$message .= '<p class="marginbot"><a href="'.$url.'" class="lightlink">Èç¹ûÄúµÄä¯ÀÀÆ÷Ã»ÓÐ×Ô¶¯Ìø×ª£¬Çëµã»÷ÕâÀï</a></p>';
				//$url = transsid($url);
			...
			"<script type=\"text/javascript\">".
			"if(history.length > (BROWSER.ie ? 0 : 1)) document.write('<a href=\"javascript:history.go(-1);\" class=\"lightlink\">·µ»Ø</a>');".
			"</script>".
			'</p>';
		}
	}
